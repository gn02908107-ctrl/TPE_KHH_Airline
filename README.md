...
- **航班明細資料表**:可依目前篩選條件即時查看原始資料,預設顯示最新資料在前
- **誤點 / 取消資料追蹤**(桃園、高雄機場皆有):記錄表定時間、實際時間,自動換算誤點分鐘數,並判斷班機是否取消(取消班機不計入誤點統計)

## 技術架構

```
GitHub Actions(每日排程,台灣時間約 00:42 自動觸發)
    │
...
**穩定性設計**:
- 爬蟲遇到連線逾時、伺服器錯誤時會自動重試(遞增等待間隔),避免官網一時不穩就讓整個排程失敗
- 資料表結構變更(例如新增誤點/取消欄位)採自動遷移,不需要手動改資料庫、也不影響既有資料
- 資料庫寫入採「目標日期整批覆寫、跨日資料逐筆比對去重」,排程重跑不會產生重複資料
- 儀表板端對目的地名稱、每日趨勢異常值都有防呆機制,單一筆資料異常不會讓整頁面壞掉
...
`.github/workflows/scrape.yml` 設定了 GitHub Actions,每天台灣時間約 00:42 自動執行(刻意避開 00:00、00:30 這種大家都愛用的整點/半點,減少 GitHub Actions 排程佇列塞車、實際執行時間被延後的機率):
...
| 航空公司 | 該航班的營運航空公司 |
| 目的地 | 航班的目的地(城市/機場) |
| 機型 | 執飛機型(已轉換為 ICAO 代碼) |
| 日期 | 航班日期 |
| 類型 | 離境 / 到場 |
| 表定時間 | 原定時間(桃園:OTime;高雄離境:STD、到場:STA) |
| 實際時間 | 實際時間(桃園:RTime;高雄離境:ATD、到場:ATA) |
| 誤點分鐘 | 實際時間 − 表定時間(已處理跨午夜情況);取消班機不計算此值 |
| 是否取消 | 0/1。桃園依 `Memo`/`CurrentStatus` 欄位是否出現「取消」字樣判斷;高雄依實際時間欄位(ATD/ATA)是否為空判斷 |
| 航班狀態、備註 | 僅桃園機場資料有此欄位,保留 API 原始的狀態文字供除錯參考 |

> 目的地的經緯度與地區/國家分類,是額外整理的對照表(`app.py` 中的 `DEST_INFO`),資料來源若新增未收錄的目的地名稱,儀表板會顯示提醒訊息而不會中斷執行。
>
> 舊資料(新增誤點/取消欄位之前寫入的資料)這幾欄會是 `NULL`,屬於已知限制,沒有歷史 OTime/RTime/STD/STA 可以回填。
...
- [ ] 誤點預測模型(用累積的表定/實際時間資料做時間序列或分類預測)
